...
// 部署并初始化一个新的投票合约，指定拥有者为此地址
function createVote(voteInfo, callback) {

  // 部署合约
  var voteContract = new web3.eth.Contract(contractTool.voteContractAbi);
  voteContract.deploy({
...
function getUserInfo(ethAddress, contractAddr, callback) {
  ...
  healthContract.methods.getUserInfo(ethAddress).call().then(function (result) {
    callback(1, result)
  }, function () {
    callback(0)
  });
...
function getHealthCount(contractAddr, callback) {
  ...
  healthContract.methods.getHealthCount().call().then(function (result) {
    callback(1, result)
  }, function () {
    callback(0)
  });
...
function getPublicHealthCount(contractAddr, callback) {
  ...
  healthContract.methods.getPublicHealthCount().call().then(function (result) {
    callback(1, result)
  }, function () {
    callback(0)
  });
...
function getAuthInfo(ethAddress, contractAddr, callback) {
  ...
  healthContract.methods.getAuthInfo(ethAddress).call().then(function (result) {
    callback(1, result)
  }, function () {
    callback(0)
  });
...
function getHDataByIndex(index, ethAddress, contractAddr, callback) {
  ...
  healthContract.methods.getHDataByIndex(ethAddress, index).call().then(function (result) {
    callback(1, result)
  }, function () {
    callback(0)
  });
